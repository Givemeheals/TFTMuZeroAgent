...
class player:
        ...
        if self.level < self.max_level and self.exp >= self.level_costs[self.level]:
            ...
            if 'birthday_present' in self.augment_dict:
                if not self.bench_full():
                    ...
                    y = min(x, 5)
                    name = self.pool_obj.sample(self, 1, y, False)[0]
                    ...
                    self.update_triple_catalog(self.bench[temp_slot])
            self.level_up()
    ...
    def move_bench_to_board(self, bench_x, board_x, board_y):
        ...
        if 0 <= bench_x < 9 and self.bench[bench_x] and 7 > board_x >= 0 and 4 > board_y >= 0 \
            ...
            if self.num_units_in_play < self.max_units:
                ...
                if m_champion.items and m_champion.items[0] == 'thieves_gloves':
    ...
    def move_board_to_bench(self, x, y):
        if 0 <= x < 7 and 0 <= y < 4:
            if self.bench_full():
                ...
                if self.board[x][y] and self.board[x][y].name != 'sandguard':
                    ...
                    if not self.item_bench_full(len(champ.items)):
                        while len(champ.items) > 0:
                            if champ.items[0] == 'thieves_gloves':
                                self.thieves_gloves_loc.remove([x, y])
                                champ.items = ['thieves_gloves']
    ...
    def return_item_from_bench(self, x):
        ...
        if self.bench[x]:
            ...
            if self.bench[x].items:
                ...
                if not self.item_bench_full(len(self.bench[x].items)) and 'salvage_bin' not in self.augment_dict:
                ...
                elif not self.item_bench_full(1) and self.bench[x].items[0] == "thieves_gloves" and 'salvage_bin' not \
                        in self.augment_dict:
                    self.item_bench[self.item_bench_vacancy()] = self.bench[x].items[0]
                elif 'salvage_bin' in self.augment_dict and self.bench[x].items[0] != 'thieves_gloves':
    ...
    def return_item(self, a_champion):
        ...
        if a_champion:
            ...
            if a_champion.items:
                ...
                # if I have enough space on the item bench for the number of items needed
                if not self.item_bench_full(a_champion.num_items) and 'salvage_bin' not in self.augment_dict and not \
                        a_champion.items[0] == 'thieves_gloves':
                    # Each item in possession
                    for item in a_champion.items:
                        self.item_bench[self.item_bench_vacancy()] = item
                # if there is only one or two spots left on the item_bench and thieves_gloves is removed
                elif not self.item_bench_full(1) and a_champion.items[0] == "thieves_gloves" and not 'salvage_bin' in self.augment_dict:
                    self.item_bench[self.item_bench_vacancy()] = a_champion.items[0]
                elif 'salvage_bin' in self.augment_dict and a_champion.items[0] != 'thieves_gloves':
                    for item in a_champion.items:
                        if item in list(item_builds.values()) and not self.item_bench_full(2):
                            self.item_bench[self.item_bench_vacancy()] = item_builds[item][0]
                            self.item_bench[self.item_bench_vacancy()] = item_builds[item][1]
    ...
    def thieves_gloves(self, x, y):
        ...
        if y >= 0:
            while len(self.board[x][y].items) != 1:
            ...
            while len(self.bench[x].items) != 1:
