...
class TFT_Simulator(AECEnv):
    ...
    def check_dead(self):
        ...
        for key, player in self.PLAYERS.items():
            if player:
                if player.health <= 0 and not 'last_stand' in player.augment_dict:
                    self.NUM_DEAD += 1
                    ...
                    if player.health <= 0:
                        ...
                        player.augment_dict.pop('last_stand')
                        for x in range(7):
                            for y in range(4):
                                if player.board[x][y]:
                                    player.board[x][y].last_stand = True
                        for x in range(9):
                            if player.bench[x]:
                                player.bench[x].last_stand = True
                    num_alive += 1
        return num_alive
